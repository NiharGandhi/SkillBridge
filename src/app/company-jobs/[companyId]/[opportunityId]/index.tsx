...
const OpportunityApplicantsScreen = () => {
    ...
    const { user } = useAuth();
    const { companyId, opportunityId } = useLocalSearchParams();
    ...
    useEffect(() => {
        fetchOpportunityAndApplications();
    }, [opportunityId, filter]);

    const fetchOpportunityAndApplications = async () => {
        try {
            setLoading(true);
            setProfileError(null);

            // First fetch the opportunity title
            ...
            // Then fetch applications for this opportunity
            let query = supabase
                .from('applications')
                .select(`
                    *,
    ...
    const handleViewResume = async (resumeUrl: string | null) => {
        ...
        try {
            ...
            Alert.alert('Error', 'Failed to open resume. Please try again later.');
        }
    };
    ...
    const handleUpdateStatus = async (applicationId: string, status: string) => {
        try {
            ...
            if (error) throw error;
            await fetchOpportunityAndApplications();
        } catch (error) {
...
export default OpportunityApplicantsScreen;